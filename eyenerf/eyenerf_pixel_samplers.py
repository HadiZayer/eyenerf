...
class PixelSampler:  # pylint: disable=too-few-public-methods
    ...
    def sample_method(  # pylint: disable=no-self-use
        ...
        image_height: int,
        image_width: int,
        nonzero_indices: Optional[Tensor] = None,
        ...
        """
        if isinstance(nonzero_indices, torch.Tensor):
            # nonzero_indices = torch.nonzero(mask[..., 0], as_tuple=False)
            # print(len(nonzero_indices), batch_size)
            chosen_indices = random.sample(range(len(nonzero_indices)), k=batch_size)
            # indices = nonzero_indices[chosen_indices].to(device)  # TODO: fix this hack
    ...
    def collate_image_dataset_batch(self, batch: Dict, num_rays_per_batch: int, keep_full_image: bool = False):
        ...
        collated_batch = {
            ...
            if key not in ["image_idx", "nonzero_indices", "ellipse_params"] and value is not None
        }
